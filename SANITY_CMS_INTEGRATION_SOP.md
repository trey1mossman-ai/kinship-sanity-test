...
### Lesson 15: Sanity Fields Must Affect the Website or Be Read-Only Notes

**Problem:** A `footerText` field existed in Sanity that appeared editable but changes to it did NOT update the website.

**Cause:** The Footer component hardcodes the copyright dynamically using `new Date().getFullYear()`. The Sanity field was never wired up to the component.

**Impact:** Misleading UX - editors think they can change something when they can't.

**The Rule:** Every field in Sanity MUST either:
1. **Directly affect the website** - Changes in Sanity appear on the site after publish
2. **Be a read-only note** - Marked `readOnly: true` with clear description explaining it's informational only

**NEVER have editable fields that don't do anything.**

**Solution Pattern for Informational Fields:**

```typescript
// ✅ CORRECT - Read-only note field
defineField({
  name: 'footerCopyrightNote',
  title: 'Footer Copyright (Auto-Generated)',
  type: 'string',
  description: '⚠️ READ-ONLY: The copyright year is automatically generated by the website code. This field is for reference only and changes here will NOT affect the website.',
  readOnly: true,
  group: 'footer'
}),

// ❌ WRONG - Editable field that doesn't affect the site
defineField({
  name: 'footerText',
  title: 'Footer Copyright Text',
  type: 'string',
  // No readOnly, appears editable but changes do nothing!
  group: 'footer'
}),
```

**Audit Checklist for Sanity Fields:**
- [ ] Does editing this field update the website? If NO:
  - [ ] Is it marked `readOnly: true`?
  - [ ] Does the description clearly state it's informational?
  - [ ] Does the title indicate it's auto-generated or read-only?

**When to Use Read-Only Notes:**
- Auto-generated values (copyright year, build timestamps)
- Calculated fields (totals, counts)
- System information (deployment status, sync dates)
- Reference data that's managed in code
